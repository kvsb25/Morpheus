...
export const RCA_REVIEWER_SYSTEM_PROMPT =
  `That is a crucial architectural detail. When using \`.withStructuredOutput()\`, you must strip out all manual text-formatting instructions (like "Output exactly the word REJECTED") because the orchestration layer is already enforcing the JSON structure under the hood. If the prompt fights the tool parser, it can cause parsing errors or hallucinations.

Instead, the system prompt should focus entirely on **field mapping**—instructing the LLM on *how* to populate the specific fields in your Zod schema based on its logical evaluation.

Here is the revised system prompt optimized for \`.withStructuredOutput()\`. It handles the edge case of an "APPROVED" status while strictly adhering to your Zod enums.

---

### **RCA Reviewer Agent: System Prompt**
